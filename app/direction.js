...
function intro() {
  ...
  return `<header class="m-form-page__intro"><span class="m-label">Project Direction</span><h1 class="m-heading">Name the creative center.</h1><p class="m-copy m-copy--large">Keep the director's words intact. Meridian versions them and shows which Scenes still point to the earlier direction.</p><div class="m-stack"><span class="m-state m-state--current">Current V${version(current.version)}</span><span class="m-meta">SET BY ${escape(String(current.setBy || "NOT RECORDED").toUpperCase())}</span></div></header>`;
}
...
function render() {
  locationBar.innerHTML = `<nav class="m-breadcrumb" aria-label="Breadcrumb"><a href="./tour.html?tour=${escape(TOUR_ID)}">${escape(view.tour.name)}</a><span aria-hidden="true">/</span><span class="m-breadcrumb__current">Project Direction</span></nav>`;

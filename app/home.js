...
function startTour() {
  ...
  root.innerHTML = `<section class="m-empty-state m-empty-state--action" aria-labelledby="start-tour-heading">
      <div class="m-empty-state__visual" aria-hidden="true">${emptyGlyph("tour")}<span class="m-empty-state__calibration">Tour / Not started</span></div>
      <div class="m-empty-state__body">
        ...
        <p class="m-copy m-copy--large">Meridian keeps one tour's creative direction, Scene requests, versions, and approvals in one place, so everyone works from the same record.</p>
...
function setupLines(tour, assignments) {
  ...
  const lines = [
    ...
    },
    {
      ...
      detail: dates.length ? `${dates.length} ${dates.length === 1 ? "date" : "dates"} added` : "No dates yet",
      href: `./tour.html?tour=${encodeURIComponent(TOUR_ID)}#tour-run-heading`,
    },
    {
      label: "Production details",
      filled: Boolean(setup && String(setup.words || "").trim()),
      detail: setup && String(setup.words || "").trim() ? "Recorded" : "No production details yet",
      href: `./tour.html?tour=${encodeURIComponent(TOUR_ID)}#setup-heading`,
    },
    {
      label: "Scenes",
      filled: assignments.length > 0,
      detail: assignments.length ? `${assignments.length === 1 ? "First Scene requested" : `${assignments.length} Scenes requested`}` : "No Scenes yet",
...
function setupShape(user, tour, assignments) {
  return `<header class="m-home__header"><div class="m-home__header-copy"><span class="m-label">Today</span><h1 class="m-heading">Welcome, ${escape(firstName(user))}</h1><p class="m-copy m-copy--large">${escape(tour.name)} is set up. Add what you know, and request the first Scene whenever you are ready.</p></div><a class="m-button m-button--primary" href="./request.html?tour=${escape(TOUR_ID)}">Request a Scene</a></header>
    <section class="m-stack" aria-labelledby="tour-holds-heading">
      <div class="m-section-lead"><div class="m-stack"><span class="m-label">What this tour holds</span><h2 id="tour-holds-heading" class="m-section-heading">The tour so far</h2></div></div>
      <div class="m-readiness-list">${setupLines(tour, assignments)}</div>
      <p class="m-copy">None of this has to be filled in first. A Scene can be requested with none of it.</p>
    </section>`;
}

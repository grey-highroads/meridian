...
## Three rulings, 2026-08-30, third session

Recorded against the committed tree at `e3ae5182`. A correction to the second session of the same day, made after Grey read the four instruments on the live app and could not find the way back to an answer.

**An answer that exists outranks asking for it again.** The 2026-08-30 door ruling said an instrument that has run becomes the way back to its answer. It did not say with what weight, and the build answered that question badly. The door shipped as `m-button--quiet`, which in `app/design/components.css` is a transparent background, a transparent border, no shadow, and muted text. So it rendered as small muted uppercase text sitting between two other pieces of small muted uppercase text, the run label and the showing state, and it was the only control on the card. Meanwhile the ask kept `m-button--primary` and its cobalt edge. The loudest control on a card asked a question already answered and the quietest text on it was the answer.

The correction: once a job holds a stored run, the way back to that answer is the primary control on that card and asking again steps down to the plain control treatment. The run label is metadata and never shares a type treatment with a control beside it. Arriving where you already are is a state rather than a control, which is the grammar the run history already uses. `answerDoor` and `askTreatment` in `app/intelligence.js`.

**The four instruments share one footer skeleton.** Also missing from the second-session ruling, and the reason the four read as badly composed. The footer held four different control shapes, a select with a button, a lone button, a select with a button, and a state span, and the body row above absorbed the difference, so no two controls in a row landed at the same height. The answer row existed only on cards that had run, so two cards carried one and two did not and every bottom edge was ragged differently.

The correction: the ask sits in a slot of its own with its control on the floor of that slot, and the way back sits in a row beneath it. Where subgrid is available the row tracks are shared across the field, so two cards side by side agree on the height of both rows and a card with no answer holds that row open. Stacked in one column no card sits beside another, the tracks are sized by each card alone, and nothing is reserved. The fallback for a browser without subgrid holds the same shape through a minimum height and a reserved element, and pays the reserved room in one column too. `app/design/patterns.css` and `askInstrument` in `app/intelligence/asks-view.js`.

**The instruments earn their size on a first visit and not after.** They were sized for someone who has never asked anything and never gave that size back. Once any instrument on the page holds an answer, the four tighten: padding, internal gaps, heading scale, and copy scale. Spacing only. Every word an instrument says on a first visit it keeps saying afterwards, and no control changes what it does. Nothing is reserved on a first visit either, because no card sits beside one holding an answer and the room belongs to the page.

**The 932 pixel check, recorded as a pass.** The 2026-08-28 ruling that an answer announces itself in the first viewport was measured rather than assumed, because Chrome gives the live tab 932 usable vertical pixels and the local frame is 1000. Rendered from the production build at 1440 by 932 in the Higher Roads role, with all three jobs holding a run, the label announcing the answer sat at 817 pixels on the committed tree and sits at 802 after this change. At 390 by 844 it sat at 1519 and sits at 1473. Both widths improve and neither regresses. At phone width the label remains far below the fold, which the 2026-08-28 ruling did not cover and this change does not fix.

What may not change and did not: no colour separates the four asks, the marks stay inside their headings, four instruments stay peers so job four keeps its place, cross-job movement stays out of the answer head, run history stays within its job, there is no verdict, meter, or score, the 44rem copy measure and the 70rem canvas hold, and Copy and Download stay at both scales rendered from the stored run.

## One ruling, 2026-09-04

Recorded against the committed tree at `cfbc7068`. Raised by a research sweep on how tours other than the pilot's are shaped, and settled after the cost of the alternative was read from the tree.

**The container keeps its stored name and carries a label and a shape.** Meridian's middle layer is not only a tour. A residency, a festival run, and a projection mapping job with no music and no touring in it all need the same four things the layer provides: one direction version that assignments are written against, one set of technical facts, one cast of people with permissions, and one close where facts are recorded. A tour is one shape that layer takes.

Renaming it was considered and refused. The word appears 2,966 times across 73 of the 96 JS files under `src`, `api`, `app`, and `test`. `tourPathFor` in `src/tour/store.js` builds every stored path from it. `jobIdFor` in `src/tour/brief.js` builds the job id as `${tourId}--${assignmentId}`, and that id has already crossed the seam into production's receiver and into the acknowledgement facts `appendFact` in `src/tour/scene-record.js` writes. `tourId` and `tourDirection` are fields inside briefs that are already frozen, which the 2026-08-27 entry in `docs/deferred-work.md` established are permanent. A rename would land in stored paths, frozen briefs, and an identifier another system holds, which are the three worst places a naming change can land.

So the stored word stays. The container gains a display label, which is what the client calls this piece of work, and a shape, which says how many technical profiles to expect and who supplies them. A tour has many and we define them. A residency or an install has one, fixed. A festival run has many and a promoter supplies them. Everything else about the layer is identical across shapes. Interface copy reads the label, never the stored word.
...
For the static ones the word came out rather than in. The nav landmark names Meridian. Page titles read Project, Direction, and Knowledge. Knowledge replaced Artist Brain because Brain is system vocabulary reaching a user, which the writing rules already refused; the page says what it holds in its own description.

Corrected 2026-09-04, in the commit that carries it. The rail first read Details, and the paragraph above first said Grey ruled that word. Both were wrong. The architect chose Details when the builder proposed dropping the category word, without asking what the page holds, and Grey rejected it: every page has details, so the word names nothing. The rail now reads Project, which names the thing the Scenes and the Reviews hang off. Project also moved to second position, after Home, because it is where the first work happens on a new account and it sat furthest from a new person's starting point. Home's two links into it, the error message on the project page, and the shell test's label list follow the same word. The general risk in Project is that it invites task and owner expectations the roadmap refuses; nothing behaves differently because of the word, and the record's own label still carries meaning where a record is in scope.
...
## Projects without a subject, 2026-09-05, fourth session

Recorded against the committed tree at `31aecb53`, executing steps 1 and 2 of `docs/meridian-roadmap-phase-2.md`. These are consequences of rulings already made, written down because each one changes something a person reads.

**Identities come from the artist being imported.** `src/artist/parse-intake.js` held three identities as a module constant, taken from the first artist and offered to every artist after him, his side project among them. The intake files already stated them: one line in the header of `01-sources.md` joins the codes used in the source and claim tables to the names used as headings in `03-findings.md`. That line is what the parser reads now. An intake stating no line is imported with a main stage and a shared bin. `docs/intake-playbook.md` carries the requirement.

The same constant was doing a second job in `src/artist/service.js`, where grouping iterated it to build the Artist Brain page. An artist whose identities were not on that list would have had every finding filtered out and nothing would have said one was missing. Grouping reads the record's own identities now, and a finding carrying an identity the record does not name is shown under a heading read from its id rather than dropped. The failure this fixes was silent, which is why the register entry it left behind requires its eventual fix to make absence say so.

**A project holds a subject or it does not, and nothing branches on which.** Three refusals came out. `create-account` no longer demands a first artist name, so an account is an account. `create-tour` no longer demands an `artistId` and no longer verifies one that was never given; an id that is given is verified exactly as before. `parseTour` no longer demands an Artist line. A project with no subject stores `artistId` as null.

The frozen brief keeps its shape. `artistId` compiles as null and `artistContext` as an empty list rather than either being dropped, so the sidecar's field set does not change and the production receiver's checks are untouched. What changes is the rendered document, which is re-rendered from the stored object on every read: a job with no subject gets no artist section and its avoid fallback reads without naming an artist. A heading over nothing names nothing, which is the same reason the rail stopped reading Details.

What this does not license. No kind, shape, or type key on the project record; the guard test that forbids one stays green. No second creation screen for a job with no subject. No branch on a display label. The brief contract does not change shape, which is step 6 of the phase 2 roadmap and not this work.

## Intelligence degrades by capability, 2026-09-05, fifth session

Recorded against the committed tree at `310c2f45`, building the amended ruling of the third session. Each instrument states what it needs and runs when it has it.

**Which instruments ask for the research, and which never do.** `loadBrain` refuses when a subject's research is not approved, and that refusal is unchanged. What changed is who calls it. Scene ideas and the artboard check ask through `loadBrainIfAny`, which returns nothing when the project names no subject and calls `loadBrain` unchanged when it does. The direction comparison still calls `loadBrain` directly, because comparing a direction against a history needs a history, and it refuses a project with no subject in its own words before it gets there.

**No empty artist fields reach the model.** A subjectless run does not send the same prompt with the artist parts blank. It sends a different prompt that carries no artist material and no assumptions about what kind of job this is: no artist, no song, no band, no setlist, no stage. What travels is the direction, the request, the production setup, and for the artboard check the board itself, the concept, and the brief's avoid list. A test scans the assembled prompt for those words and fails on any of them.

**The path for a project that has a subject is byte for byte unchanged.** Both prompts branch on one field and the branch is only taken when the field says there is no subject. This was checked by loading the modules as they stood at `310c2f45` beside the current ones and comparing the assembled model requests and the check results on the same context.

...
Job: a setting on a form that is on or off, read at a glance beside its label.
Missing state: there is no accepted pattern for a checkbox or switch with a
label next to it. `app/design/components.css` gives the bare input an accent
colour under `.m-ui` and stops there.
Where it occurs: the approve setting on a person's row in Admin, which is the
first on-or-off setting in the product. Built 2026-09-05 with `m-field` holding
an `m-cluster` and an `m-help` line, which is the nearest accepted set.
Status: requested.
...
Corrected 2026-09-08, in the commit that carries it. This entry first said two
reads still stood in `app/tour.js` and that they might be the job that brings the
control back. A rendered review found six, not two: the aside heading and the
breadcrumb link in `app/tour.js`, and the eyebrow, the heading, and the
disclosure summary of the foundation panel in `app/home.js`, plus the
introduction calibrations and its last button. All of them now read project.
Nothing in `app/` reads the stored word, so the control stays off until something
else needs it. `tourLabel` and `TOUR_LABEL` stay exported from `app/label.js`
with no reader, because the field is still stored and a future screen may want
it.
